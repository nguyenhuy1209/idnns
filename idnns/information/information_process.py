...
def calc_information_for_epoch(iter_index, interval_information_display, ws_iter_index, bins, unique_inverse_x,
                               ...
                               len_unique_a, pys, pxs, py_x, pys1, x, y, model_path, input_size, layerSize,
                               calc_vartional_information=False, calc_information_by_sampling=False,
                               calc_full_and_vartional=False, calc_regular_information=True, calc_multivariate_information=False, num_of_samples=100,
                               sigma=0.5, ss=[], ks=[]):
	...
	# Calc infomration of only subset of the neurons
		...
		params = np.array(
			[sth(ws_iter_index[i], x, y, random_indices) for i in range(len(ws_iter_index))]
		)
	...
	if calc_parallel:
		params = np.array(Parallel(n_jobs=NUM_CORES
		                             ...
		                             (i, interval_information_display, ws[i], bins, unique_inverse_x, unique_inverse_y,
		                              ...
		                              pxs, p_y_given_x, pys1, x, label, model.save_file, x.shape[1], layerSize)
		                             for i in range(len(ws)))) # iterate epochs
	else:
