...
def calc_information_for_layer(
    data, bins, unique_inverse_x, unique_inverse_y, pxs, pys1
):
    bins = bins.astype(np.float32)
    digitized = bins[np.digitize(np.squeeze(data.reshape(1, -1)), bins) - 1].reshape(
        len(data), -1
    )
    b2 = np.ascontiguousarray(digitized).view(
        np.dtype((np.void, digitized.dtype.itemsize * digitized.shape[1]))
    )
    unique_array, unique_inverse_t, unique_counts = np.unique(
        b2, return_index=False, return_inverse=True, return_counts=True
    )
    p_ts = unique_counts / float(sum(unique_counts))
    PXs, PYs = np.asarray(pxs).T, np.asarray(pys1).T
    local_IXT, local_ITY = calc_information_from_mat(
        PXs, PYs, p_ts, digitized, unique_inverse_x, unique_inverse_y, unique_array
    )
    return local_IXT, local_ITY


def calc_information_sampling(
    data,
    bins,
    pys1,
    pxs,
    ...
    if calc_DKL:
        pxy_given_T = np.array(
            [
                calc_probs(i, unique_inverse_t, label, b, b1, len_unique_a)
...
def calc_information_for_layer_with_other(
    data,
    bins,
    unique_inverse_x,
    unique_inverse_y,
    ...
    local_IXT, local_ITY = calc_information_sampling(
        data,
        bins,
        pys1,
        pxs,
    ...
    number_of_indexs = int(data.shape[1] * (1.0 / 100 * percent_of_sampling))
...
def sth(data, x, y, random_indices):
    alpha = 1.01
    h = 0.5
    ...
    sigmaY = m ** (-1 / (4 + y.shape[1]))
    sigmaT = m ** (-1 / (4 + data.shape[1]))

    sigmaX = h * sigmaX
    sigmaY = h * sigmaY
    sigmaT = h * sigmaT

    local_IXT = calc_multivariate_information(x, data, sigmaX, sigmaT, alpha)
    local_ITY = calc_multivariate_information(y, data, sigmaY, sigmaT, alpha)

    # print('local_IXT', local_IXT)
    # print('local_ITY', local_ITY)
...
def calc_by_sampling_neurons(ws_iter_index, num_of_samples, label, sigma, bins, pxs):
    ...
    for j in range(len(ws_iter_index)):
        ...
        unique_array, unique_indices, unique_inverse_x, unique_counts = np.unique(
            b, return_index=True, return_inverse=True, return_counts=True
        )
        b_y = np.ascontiguousarray(labels).view(
            np.dtype((np.void, labels.dtype.itemsize * labels.shape[1]))
        )
        unique_array_y, unique_indices_y, unique_inverse_y, unique_counts_y = np.unique(
            b_y, return_index=True, return_inverse=True, return_counts=True
        )
        pys1 = unique_counts_y / float(np.sum(unique_counts_y))
        iter_infomration.append(
            calc_information_for_layer(
                data=new_data,
                bins=bins,
...
def calc_information_for_epoch(
    iter_index,
    ...
    calc_multivariate_information=False,
    num_of_samples=100,
    sigma=0.5,
    ss=[],
    ks=[],
    ...
    if calc_full_and_vartional:
        ...
        params_original = np.array(
            [
                calc_information_for_layer_with_other(
    ...
    # Calc infomration of only subset of the neurons
        ...
        parmas = calc_by_sampling_neurons(
            ws_iter_index=ws_iter_index,
    ...
    elif calc_multivariate_information:  # MMI from anh Tien
        random_indices = np.sort(np.random.randint(0, x.shape[0], size=400))
        params = np.array(
            [
                sth(ws_iter_index[i], x, y, random_indices)
                for i in range(len(ws_iter_index))
            ]
        )
...
def extract_probs(label, x):
    ...
    # bins = np.linspace(0, 256, 1, endpoint=False)
    # x = bins[np.digitize(np.squeeze(x.reshape(1, -1)), bins) - 1].reshape(len(x), -1)
...
def get_information(
    ...
    pys, pys1, p_y_given_x, b1, b, unique_a, unique_inverse_x, unique_inverse_y, pxs = (
        extract_probs(label, x)
    )
