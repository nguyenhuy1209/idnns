...
def plot_all_epochs(gen_data, I_XT_array, I_TY_array, axes, epochsInds, f, index_i, index_j, size_ind,
    ...
    if index_i ==axes.shape[0]-1 and index_j ==axes.shape[1]-1:
        ...
        f.suptitle('Information plane - Renyi MI - ReLU activation - 5 iterations - all layer', fontsize=25)
...
def extract_array(data, name):
    if len(data.shape) == 3:
        results = [[[data[i,j,k][name] for k in range(data.shape[2])] for j in range(data.shape[1])] for i in range(data.shape[0])]
    else:
