...
def plot_all_epochs(
    ...
    # comment these two lines if you want to draw for each layer
    ...
    for index_in_range in range(0, max_index):
        ...
        if epochsInds[index_in_range] == index_to_emphasis:
            axes[index_i, index_j].plot(
                XT,
                TY,
                marker="o",
                linestyle=None,
                markersize=19,
                markeredgewidth=0.04,
                linewidth=2.1,
                color="g",
                zorder=10,
            )
    ...
    if index_i == axes.shape[0] - 1 and index_j == axes.shape[1] - 1:
        utils.create_color_bar(
            f, cmap, colorbar_axis, bar_font, epochsInds, title="Epochs"
        )
        f.suptitle(
            "Information plane - Renyi MI - Absolute value - 5 iterations - all layer",
            fontsize=25,
        )
        f.savefig(save_name + ".jpg", dpi=500, format="jpg")


def plot_by_training_samples(
    I_XT_array,
    I_TY_array,
    axes,
    epochsInds,
    ...
    max_index = size_ind if size_ind != -1 else I_XT_array.shape[1] - 1
    cmap = plt.get_cmap("gnuplot")
    ...
    for index_in_range in range(0, max_index):
        ...
        for layer_index in range(0, I_XT_array.shape[4]):
            XT.append(
                np.mean(
                    I_XT_array[:, -1, index_in_range, nums_epoch, layer_index], axis=0
                )
            )
            TY.append(
                np.mean(
                    I_TY_array[:, -1, index_in_range, nums_epoch, layer_index], axis=0
                )
            )
        axes[index_i, index_j].plot(
            XT,
    ...
    if index_i == axes.shape[0] - 1 and index_j == axes.shape[1] - 1:
        utils.create_color_bar(
            f, cmap, colorbar_axis, bar_font, epochsInds, title="Training Data"
        )
...
def calc_velocity(data, epochs):
    ...
    for layer_index in range(data.shape[5]):
        ...
        for epoch_index in range(len(epochs) - 1):
            vx = np.mean(
                data[0, :, -1, -1, epoch_index + 1, layer_index], axis=0
            ) - np.mean(data[0, :, -1, -1, epoch_index, layer_index], axis=0)
            vx /= epochs[epoch_index + 1] - epochs[epoch_index]
            vy = np.mean(
                data[1, :, -1, -1, epoch_index + 1, layer_index], axis=0
            ) - np.mean(data[1, :, -1, -1, epoch_index, layer_index], axis=0)
...
def update_line_each_neuron(
    ...
    if len(axes) > 1:
        axes[1].plot(
            epochsInds[:num], 1 - np.mean(accuracy_test[:, :num], axis=0), color="g"
        )
        if print_loss:
            axes[1].plot(
                epochsInds[:num], np.mean(loss_test_data[:, :num], axis=0), color="y"
            )
...
def plot_animation(name_s, save_name):
    ...
    f.subplots_adjust(
        left=0.14, bottom=0.1, right=0.928, top=0.94, wspace=0.13, hspace=0.55
    )
    colors = LAYERS_COLORS
    # new/old version
    if False:
    ...
    """
    if print_loss:
        loss_train_data = interp1d(epochsInds, np.squeeze(loss_train_data), axis=1)(
            new_x
        )
...
def plot_snapshots(
    ...
    f.subplots_adjust(
        left=0.095, bottom=0.18, right=0.99, top=0.97, wspace=0.03, hspace=0.03
    )
    # Adding axes labels
...
def load_figures(mode, str_names=None):
    ...
    if mode == 0:
        ...
        f.subplots_adjust(
            left=0.09, bottom=0.15, right=0.928, top=0.94, wspace=0.03, hspace=0.04
        )
        yticks = [0, 1, 2, 3]
        xticks = [3, 6, 9, 12, 15]
    ...
    if mode == 1:
        ...
        f.subplots_adjust(
            left=0.09, bottom=0.08, right=0.92, top=0.94, wspace=0.03, hspace=0.15
        )
    ...
    if mode == 11:
        ...
        title_strs = [
            ...
            ["Four hidden layers", "Five hidden layers", "Six hidden layers"],
        ]

        title_strs = [
            ["5 Bins", "10 Bins", "15 Bins"],
            ["20 Bins", "25 Bins", "35 Bins"],
        ]
        f, (axes) = plt.subplots(2, 3, sharex=True, sharey=True, figsize=fig_size)
        f.subplots_adjust(
            left=0.09, bottom=0.1, right=0.92, top=0.94, wspace=0.03, hspace=0.15
        )
        colorbar_axis = [0.93, 0.1, 0.035, 0.76]
        sizes = [[1010, 1010, 1017], [1020, 1700, 920]]
    # one figure
    if mode == 2 or mode == 6:
        ...
        f.subplots_adjust(
            left=0.097, bottom=0.12, right=0.87, top=0.99, wspace=0.03, hspace=0.03
        )
    ...
    if mode == 3:
        ...
        if len(str_names) == 1:
            axes = np.vstack(np.array([axes]))
        f.subplots_adjust(
            left=0.097, bottom=0.12, right=0.90, top=0.99, wspace=0.03, hspace=0.03
        )
        sizes = [[-1]]
    ...
    if mode == 4:
        ...
        axes = np.vstack(axes).T
        f.subplots_adjust(
            left=0.07, bottom=0.15, right=0.933, top=0.94, wspace=0.12, hspace=0.04
        )
        # yticks = [0, 0.2, 0.4, 0.6, 0.8, 1]
        # xticks = [1, 3, 5, 7, 9, 11]
...
def plot_figures(str_names, mode, save_name):
    ...
    for i in range(len(str_names)):
        for j in range(len(str_names[i])):
            ...
            print(I_TY_array.shape)

            # I_XT_array = I_XT_array[:, [0]]
            # I_TY_array = I_TY_array[:, [0]]
...
def plot_pearson(name):
        ...
        for k in range(len(ws)):
            ...
            for neuron in range(len(ws_current[layer])):
                ...
                for neuron_second in range(neuron + 1, len(ws_current[layer])):
                    pearson_c, p_val = sis.pearsonr(
                        ws_current[layer][neuron], ws_current[layer][neuron_second]
                    )
    ...
    # Plot the coeff
    axes.bar(
        np.arange(1, 7), np.abs(np.array(pearson_mean)) * np.sqrt(sizes), align="center"
    )
...
def extract_array(data, name):
    if len(data.shape) == 3:
        results = [
            [
                [data[i, j, k][name] for k in range(data.shape[2])]
        ...
        results = [
            [data[j, k][name] for k in range(data.shape[1])]
...
def update_bars_num_of_ts(num, p_ts, H_Xgt, DKL_YgX_YgT, axes, ind_array):
    ...
    axes[0].set_title(
        "Number of Ts in every layer - Epoch number - {0}".format(ind_array[num])
    )
...
def update_bars_entropy(num, H_Xgt, DKL_YgX_YgT, axes, ind_array):
    ...
    axes[0].set_title(
        "H(X|T) in every layer - Epoch number - {0}".format(ind_array[num])
    )
    axes[0].set_xlabel("Layer Number")
    axes[0].set_ylabel("# of Ts")
...
if __name__ == "__main__":
    ...
    if action == TIME_STEMPS or action == MOVIE:
        ...
        name_s = (
            prex2
            + "g_layerSizes=10,7,5,4,3_LastEpochsInds=9998_nRepeats=40_batch=3563_DataName=var_u_nEpoch=10000_lr=0.0002_nEpochInds=964_samples=1_nDistSmpls=1/"
        )
        name_s = (
            prex2
            + "r_DataName=MNIST_sampleLen=1_layerSizes=400,200,150,60,50,40,30_lr=0.0002_nEpochInds=677_nRepeats=1_LastEpochsInds=1399_nDistSmpls=1_nEpoch=1400_batch=2544/"
        )
        ...
        if action == ALL_LAYERS:
            ...
            str_names = [
                [
                    prex + "ff3_5_198.pickle",
                    prex + "ff3_4_198.pickle",
                    prex + "ff3_3_198.pickle",
                ],
                [
                    prex + "ff3_2_198.pickle",
                    prex + "ff3_1_198.pickle",
                    prex + "ff4_1_10.pickle",
                ],
            ]
            str_names[1][2] = (
                prex2
                + "g_layerSizes=10,7,5,4,4,3_LastEpochsInds=9998_nRepeats=20_batch=3563_DataName=var_u_nEpoch=10000_lr=0.0004_nEpochInds=964_samples=1_nDistSmpls=1/"
            )
            str_names = [
                [
                    prex2
                    + "nbins8_DataName=var_u_sampleLen=1_layerSizes=10,7,5,4,3_lr=0.0004_nEpochInds=964_nRepeats=5_LastEpochsInds=9998_nDistSmpls=1_nEpoch=10000_batch=4096/",
                    prex2
                    + "nbins12_DataName=var_u_sampleLen=1_layerSizes=10,7,5,4,3_lr=0.0004_nEpochInds=964_nRepeats=5_LastEpochsInds=9998_nDistSmpls=1_nEpoch=10000_batch=4096/",
                    prex2
                    + "nbins18_DataName=var_u_sampleLen=1_layerSizes=10,7,5,4,3_lr=0.0004_nEpochInds=964_nRepeats=5_LastEpochsInds=9998_nDistSmpls=1_nEpoch=10000_batch=4096/",
                ],
                [
                    prex2
                    + "nbins25_DataName=var_u_sampleLen=1_layerSizes=10,7,5,4,3_lr=0.0004_nEpochInds=964_nRepeats=5_LastEpochsInds=9998_nDistrSmpls=1_nEpoch=10000_batch=4096/",
                    prex2
                    + "nbins35_DataName=var_u_sampleLen=1_layerSizes=10,7,5, 4,3_lr=0.0004_nEpochInds=964_nRepeats=5_LastEpochsInds=9998_nDistSmpls=1_nEpoch=10000_batch=4096/",
                    prex2
                    + "nbins50_DataName=var_u_sampleLen=1_layerSizes=10,7,5,4,3_lr=0.0004_nEpochInds=964_nRepeats=5_LastEpochsInds=9998_nDistSmpls=1_nEpoch=10000_batch=4096/",
                ],
            ]
