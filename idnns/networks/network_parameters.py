...
def get_default_parser(num_of_samples=None):
    ...
    parser.add_argument(
        "-name", "-na", dest="name", default="net", help="The name to save the results"
    )
    ...
    parser.add_argument(
        "-nDistSmpls", "-nds", dest="nDistSmpls", default=1, type=int, help="S"
    )
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="The activation function of the model 0 for tanh 1 for RelU",
    )
    ...
    parser.add_argument(
        ...
        "-iid",
        dest="interval_information_display",
        default=30,
        type=int,
    ...
    args.inds = [
        map(int, inner.split(",")) for inner in re.findall("\[(.*?)\]", args.inds)
    ]
...
def select_network_arch(type_net):
    ...
    if type_net == "1":
        ...
        layers_sizes = [
            list(map(int, inner.split(",")))
            for inner in re.findall("\[(.*?)\]", type_net)
        ]
    return layers_sizes
