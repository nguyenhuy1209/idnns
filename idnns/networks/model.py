...
class Model:
	...
	def initilizae_layer(self, name_scope, row_size, col_size, activation_function, last_hidden):
		...
		weights = get_scope_variable(name_scope=name_scope, var="weights",
		                             ...
		                             initializer=tf.compat.v1.truncated_normal_initializer(mean=0.0, stddev=1.0 / np.sqrt(
			                             float(row_size))))
	...
	@property
	def optimize(self):
		optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.learning_rate_local).minimize(self.cross_entropy)

		return optimizer

	@lazy_property
	def x(self):
		return tf.compat.v1.placeholder(tf.float32, shape=[None, self.input_size], name='x')

	@lazy_property
	def labels(self):
		return tf.compat.v1.placeholder(tf.float32, shape=[None, self.num_of_classes], name='y_true')

	@lazy_property
	def accuracy(self):
		correct_prediction = tf.equal(tf.argmax(input=self.prediction, axis=1), tf.argmax(input=self.labels, axis=1))
	...
	def cross_entropy(self):
		cross_entropy = tf.reduce_mean(
			input_tensor=-tf.reduce_sum(input_tensor=self.labels * tf.math.log(tf.clip_by_value(self.prediction, 1e-50, 1.0)), axis=[1]))
