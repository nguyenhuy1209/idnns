...
class informationNetwork:
    ...
    def __init__(self, rand_int=0, num_of_samples=None, args_additional=None):
        args = netp.get_default_parser(num_of_samples)
        if args_additional is not None:
            args.__dict__.update(args_additional)
        ...
        self.train_samples = np.linspace(1, 100, 199)[
            [[x * 2 - 2 for x in index] for index in args.inds]
        ]
        # The indexs that we want to calculate the information for them in logspace interval
        self.epochs_indexes = (
            np.unique(
                np.logspace(
                    np.log2(args.start_samples),
                    np.log2(args.num_epochs),
                    args.num_of_samples,
                    dtype=int,
                    base=2,
                )
        ...
        self.data_sets = load_data_numpy(self.name, args.random_labels)
        ...
        (
            ...
            np.zeros(
                (
                    self.num_of_repeats,
                    len(self.layers_sizes),
                    len(self.train_samples),
                    len(self.epochs_indexes),
                )
            )
            for _ in range(6)
        ]
        ...
        self.name_to_save = (
            args.name + "_" + "_".join([str(i) + "=" + str(params[i]) for i in params])
        )
        (
    ...
    def run_network(self):
        ...
        if self.run_in_parallel:
            results = Parallel(n_jobs=NUM_CORES)(
                delayed(nn.train_network)(
                    ...
                    self.train_samples,
                    self.interval_accuracy_display,
                    self.calc_information,
                    self.calc_information_last,
                    self.num_of_bins,
        ...
        for i in range(len(self.train_samples)):
            for j in range(len(self.layers_sizes)):
                for k in range(self.num_of_repeats):
                    index = (
                        i * len(self.layers_sizes) * self.num_of_repeats
                        + j * self.num_of_repeats
                        + k
                    )
            ...
            self.information = np.array(
                [
                    inn.get_information(
                        ...
                        self.args.num_of_bins,
                        self.args.interval_information_display,
                        self.epochs_indexes,
                    )
                    for i in range(len(self.train_samples))
                    for j in range(len(self.layers_sizes))
