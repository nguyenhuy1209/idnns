...
def build_model_pytorch(activation_function, layerSize, input_size, num_of_classes, learning_rate_local, save_file, covn_net):
	"""Bulid specipic model of the network
	Return the network model
	"""
	model = mo.Model(input_size, layerSize, num_of_classes, learning_rate_local, save_file, int(activation_function),
...
def train_and_calc_inf_network(i, j, k, layerSize, num_of_epochs, learning_rate_local, batch_size, indexes, save_grads,
	...
	network = train_network_pytorch(layerSize, num_of_epochs, learning_rate_local, batch_size, indexes, save_grads,
...
def train_network_pytorch(layerSize, num_of_epochs, learning_rate_local, batch_size, indexes, save_grads,
                  data_sets_org, model_type, percent_of_train, interval_accuracy_display,
                  name, covn_net):
	# initialize data
	ws, estimted_label, gradients, information, models, weights = [[None] * len(indexes) for _ in range(6)]
	loss_func_test, loss_func_train, test_prediction, train_prediction = [np.zeros((len(indexes))) for _ in range(4)]
	...
	# create dataset
	train_dataloader, test_dataloader, all_data_dataloader, input_size, num_of_classes = data_shuffle_pytorch(data_sets_org, percent_of_train, batch_size)

	# build model
	model = mo.Model(model_type, layerSize, input_size, num_of_classes, name, covn_net).to(device)
	...
	optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate_local, momentum=0.95)
	...
	for epoch in range(num_of_epochs):
		...
		# Print accuracy
		if np.mod(epoch, interval_accuracy_display) == 1 or interval_accuracy_display == 1:
			train_acc, test_acc = get_accuracy_pytorch(train_dataloader, test_dataloader, model, accuracy_metric, device)
			print('Epoch {0} - Test Accuracy: {1:.3f} Train Accuracy: {2:.3f}'.format(epoch, test_acc, train_acc))
	...
	for i, data in enumerate(dataloader):
		...
		for layer_index in range(len(layers)):
			...
			if i == 0:
				...
				w_temp[layer_index] = np.concatenate((w_temp[layer_index], layer), axis=0)
	return w_temp


def get_accuracy_pytorch(train_dataloader, test_dataloader, model, accuracy_metric, device):
	model.to(device)
